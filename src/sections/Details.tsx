...
import { Globe, Hash, Mail, Phone, Star } from 'lucide-react';
    ...
    <section className="flex flex-col gap-3">
      <div className="flex items-start justify-between gap-4">
        <div className="flex flex-1 items-start gap-3">
          ...
          <div className="my-auto flex-1">
            ...
            <div className="mt-2 flex flex-wrap gap-2 text-gray-400">
              <Item icon={Globe} text="www.tonythetailor.com" />{' '}
              <span className="hidden md:block">|</span>
              <Item icon={Mail} text="tonytailor@example.com" />{' '}
              <span className="hidden md:block">|</span>
              <Item icon={Phone} text="+91 9876543210" />{' '}
              <span className="hidden md:block">|</span>
        ...
        <div className="flex shrink-0 flex-col items-end gap-1">
          <div className="flex items-center gap-2 text-amber-500 md:gap-4">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star key={i} className="size-3 fill-current sm:size-4" />
            ))}
          </div>
          <p className="text-xs text-green-500 md:text-sm">Online Store</p>
        </div>
      </div>
